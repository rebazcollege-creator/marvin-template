...
/** The morning brief — MARVIN's written "what needs you today", generated by the
 *  heartbeat before Rebaz opens the app. forDate scopes it to one calendar day. */
type BriefSnap = { at: number; forDate: string; text: string };
const triageState: {
  inbox: TriageSnap<InboxTriage> | null;
  slack: TriageSnap<SlackTriage> | null;
  brief: BriefSnap | null;
  inboxInflight: Promise<InboxTriage> | null;
  slackInflight: Promise<SlackTriage> | null;
  briefInflight: Promise<BriefSnap> | null;
} = { inbox: null, slack: null, brief: null, inboxInflight: null, slackInflight: null, briefInflight: null };
...
function persistTriage(): void {
  try {
    writeFileSync(TRIAGE_CACHE_FILE, JSON.stringify({ inbox: triageState.inbox, slack: triageState.slack, brief: triageState.brief }));
  } catch { /* best-effort cache, never fatal */ }
}
function hydrateTriage(): void {
  try {
    if (!existsSync(TRIAGE_CACHE_FILE)) return;
    const j = JSON.parse(readFileSync(TRIAGE_CACHE_FILE, 'utf8')) as { inbox?: TriageSnap<InboxTriage>; slack?: TriageSnap<SlackTriage>; brief?: BriefSnap };
    if (j.inbox?.data?.connected) triageState.inbox = j.inbox;
    if (j.slack?.data?.connected) triageState.slack = j.slack;
    if (j.brief?.text) triageState.brief = j.brief;
  } catch { /* ignore a corrupt cache file */ }
}

/** Local calendar day as YYYY-MM-DD — the brief is scoped to one day. */
...
/**
 * Rebaz's days off (0=Sun … 6=Sat). Read from his saved settings in the kv store so
...
async function scheduledTriageRefresh(): Promise<void> {
  ...
  try {
    ...
    // Once triage is fresh, build today's morning brief — the payoff of the heartbeat.
    // Only on a work day, and only once per day (first waking-hour tick generates it).
...
const BRIEF_SYSTEM =
  `You are MARVIN writing Rebaz's morning brief (he is a journalist/editor with three jobs and ADHD). ` +
  `Using ONLY the data below — his already-triaged inbox and Slack (things that need him), today's calendar, ` +
  `and overdue/due-today Trello cards — write what genuinely needs him TODAY.\n` +
...
/** Stale-while-revalidate brief: return today's instantly; regenerate in the background
 *  if it's missing or from a previous day. Never blocks Home. */
async function getMorningBrief(): Promise<BriefSnap> {
  const snap = triageState.brief;
  const isToday = snap?.forDate === todayKey();
  const kick = () => {
    if (triageState.briefInflight) return;
    triageState.briefInflight = refreshMorningBrief().finally(() => { triageState.briefInflight = null; });
  };
...
const server = createServer(async (req, res) => {
  ...
  // The morning brief — MARVIN's "what needs you today", built by the heartbeat before
  // Rebaz opens the app. Served stale-while-revalidate, so Home never waits on the model.
  if ((req.method === 'GET' || req.method === 'POST') && req.url?.startsWith('/brief')) {
    try {
      if (req.method === 'POST') lastLearned = readLearned(await readBody(req));
